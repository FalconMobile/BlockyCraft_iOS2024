		...
		void LateUpdate () {
			...
			if (env != null && (UnityEngine.Object)env.characterController != null && env.input != null) {
				...
				if (env.input.GetButtonDown (InputButtonNames.Action)) {
					if (collider != null) {
						VoxelPlayInteractiveObject obj = collider.GetComponent<VoxelPlayInteractiveObject> ();
						if (obj == null) {
							VoxelPlayInteractiveObject[] objs = collider.GetComponentsInChildren<VoxelPlayInteractiveObject>();
