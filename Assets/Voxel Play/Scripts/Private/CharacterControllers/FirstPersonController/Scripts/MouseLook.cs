...
namespace VoxelPlay
{
	...
	{
		public float XSensitivity = 1.7f;
		public float YSensitivity = 1.7f;
		...
		{
			if (!m_cursorIsLocked || m_Input == null || !m_Input.enabled)
			{
				return;
			}
			if (orbitMode)
		...
		{
			if (VoxelPlayEnvironment.instance.initialized)
			{
		...
		{
			if (!VoxelPlayEnvironment.instance.input.enabled)
			{
				return;
			}
			if (m_cursorIsLocked)
