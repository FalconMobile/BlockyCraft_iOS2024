	...
	public partial class VoxelPlayFirstPersonController :  VoxelPlayCharacterControllerBase {
        ...
        void UpdateConstructorMode() {
            if (env.constructorMode) {
                ...
                Vector3d gridPos = buildingPosition + new Vector3d(0, size.y / 2, 0);
        ...
        public void LoadModel(ModelDefinition model) {
            ...
            // Loads model content
            Vector3d pos = buildingPosition - new Vector3(loadModel.offsetX, loadModel.offsetY, loadModel.offsetZ); // ignore offset
            env.ModelPlace(pos, loadModel);
        }
            ...
            for (int y = 0; y < size.y; y++) {
                for (int z = 0; z < size.z; z++) {
                    for (int x = 0; x < size.x; x++) {
                        Vector3d destroyPosition = buildingPosition + new Vector3 (x - size.x / 2, y, z - size.z / 2);
                        env.VoxelDestroy(destroyPosition);
                    }
                }
            }
        }
                ...
                for (int z = 0; z < sz; z++) {
                    for (int x = 0; x < sx; x++) {
                        ...
                        if (!newContents[voxelIndex].isEmpty) {
                            Vector3d placePos = buildingPosition + new Vector3 (x - size.x / 2, y, z - size.z / 2);
