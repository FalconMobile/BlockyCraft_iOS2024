...
namespace VoxelPlay
{
    ...
    {
        void LoadGameBinaryFileFormat_14(BinaryReader br, bool preservePlayerPosition = false)
        {
        ...
        void WriteChunkData(BinaryWriter bw, VoxelChunk chunk)
        {
            ...
            while (k < chunk.voxels.Length)
            {
                if (chunk.voxels[k].hasContent == 1)
                {
                    ...
                    k++;
                    while (k < chunk.voxels.Length && chunk.voxels[k].type == voxelDefinition &&
                           chunk.voxels[k].color.r == tintColor.r && chunk.voxels[k].color.g == tintColor.g &&
                           chunk.voxels[k].color.b == tintColor.b && voxelDefinition.renderType != RenderType.Custom &&
            ...
            {
                List<KeyValuePair<int, FastHashSet<VoxelProperty>>> voxelsProperties =
                    BufferPool<KeyValuePair<int, FastHashSet<VoxelProperty>>>.Get();
                List<KeyValuePair<int, VoxelProperty>> voxelProperties =
                    BufferPool<KeyValuePair<int, VoxelProperty>>.Get();
