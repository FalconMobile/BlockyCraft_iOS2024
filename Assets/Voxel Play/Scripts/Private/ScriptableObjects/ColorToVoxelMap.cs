...
namespace VoxelPlay {
	...
	public partial class ColorToVoxelMap : ScriptableObject {
		...
		public VoxelDefinition GetVoxelDefinition (Color32 color, VoxelDefinition defaultVoxelDefinition) {
			...
			VoxelDefinition vdOpaque = null;
			for (int k = 0; k < colorMap.Length; k++) {
				if (color.r == colorMap [k].color.r && color.g == colorMap [k].color.g && color.b == colorMap [k].color.b) {
					vdOpaque = colorMap [k].voxelDefinition;
					if (color.a == colorMap [k].color.a) {
