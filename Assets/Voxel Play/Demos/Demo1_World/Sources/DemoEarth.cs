...
namespace VoxelPlayDemos
{
    ...
    public class DemoEarth : MonoBehaviour
    {
        ...
        void OnInitialized ()
        {
            ...
            VoxelPlayPlayer.instance.AddInventoryItem (VoxelPlayEnvironment.GetItemDefinition ("Torch"), 3);
            VoxelPlayPlayer.instance.AddInventoryItem (VoxelPlayEnvironment.GetItemDefinition ("Torch Red"), 2);

            // Add a shovel (no need to specify quantity it's 1 unit)
            VoxelPlayPlayer.instance.AddInventoryItem (VoxelPlayEnvironment.GetItemDefinition ("Shovel"));

            // Add a sword 
            VoxelPlayPlayer.instance.AddInventoryItem (VoxelPlayEnvironment.GetItemDefinition ("Sword"));

            // Add 20 grenades
            VoxelPlayPlayer.instance.AddInventoryItem (VoxelPlayEnvironment.GetItemDefinition ("Grenade"), 200);
        ...
        void SpecialKeys ()
        {
            env.ShowMessage ("<color=green>Press <color=yellow>O</color> to throw a ball, <color=yellow>Y</color> to summon a deer, <color=yellow>X</color> to place a brick, <color=yellow>M</color> to levitate a voxel :)</color>", 20, true);
        }
