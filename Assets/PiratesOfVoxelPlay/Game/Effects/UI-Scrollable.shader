Shader "Kronnect/PiratesOfVoxelPlay/UI/Scrollable"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        CGPROGRAM
            ...
            v2f vert(appdata_t v)
            {
		        ...
		        OUT.texcoord.xy = 0.5 + (OUT.texcoord.xy - 0.5) * _MainTex_ST.xy + _MainTex_ST.zw;
            ...
            fixed4 frag(v2f IN) : SV_Target
            {
                ...
                float2 duv = (IN.scrPos.xy / IN.scrPos.w - 0.5) * 2.0;
                duv.x *= _ScreenParams.x / _ScreenParams.y;
                float vignette = pow(saturate(4.0 - dot(duv, duv)), 8.0);
